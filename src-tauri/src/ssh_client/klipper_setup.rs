...
#[tauri::command]
pub async fn klipper_can_interface(state: State<'_, Mutex<SessionState>> ) -> Result<Vec<String>, ()>{
  let mut state = state.lock().await;
  match &mut state.seission {
    Some(session) => {
      let commands = vec![
...
#[tauri::command]
pub async fn klipper_can_uuid_list(state: State<'_, Mutex<SessionState>> ) -> Result< Vec<CanDeviceUUID>, ()>{
  ...
  match &mut state.seission {
    Some(session) => {
      ...
      for can_dev in &can_devices {
        ...
        let can_query_command = format!("{} {}/scripts/canbus_query.py {}", klipper_python_path, klipper_path, can_dev);
        ...
        if let Ok(uuid_res) = can_uuid_res {
          ...
          can_device_uuids.push(CanDeviceUUID{device_name:can_dev.to_string(), can_uuids:canbus_uuids});
        }
      }
